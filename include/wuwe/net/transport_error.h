...
inline constexpr bool is_connection_error(transport_error code) noexcept {
    return code == transport_error::couldnt_connect ||
           code == transport_error::send_error ||
           code == transport_error::recv_error ||
           code == transport_error::got_nothing ||
           code == transport_error::no_connection_available;
}

inline constexpr bool is_tls_error(transport_error code) noexcept {
    return code == transport_error::ssl_connect_error ||
           code == transport_error::ssl_certproblem ||
           code == transport_error::ssl_cipher ||
           code == transport_error::peer_failed_verification ||
           code == transport_error::use_ssl_failed ||
           code == transport_error::ssl_cacert_badfile ||
           code == transport_error::ssl_shutdown_failed ||
           code == transport_error::ssl_crl_badfile ||
           code == transport_error::ssl_issuer_error ||
           code == transport_error::ssl_pinnedpubkeynotmatch ||
           code == transport_error::ssl_invalidcertstatus ||
           code == transport_error::ssl_clientcert;
}
...
[[nodiscard]]
inline std::error_code make_error_code(transport_error code) noexcept {

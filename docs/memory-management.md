...
Memory tools 的目的，是让 agent 从“被框架被动增强上下文”升级到“能在明确授权下主动管理长期记忆”。没有 memory tools 时，长期记忆主要由应用代码调用 `remember_long_term()` 写入；模型只能使用 `memory_context::augment()` 注入的上下文。加入 memory tools 后，模型可以通过显式 tool call 请求保存或搜索长期记忆，但最终仍由应用绑定的 `memory_context`、scope 和 policy 决定是否执行。

### Stateful vs Stateless Tools

现有 `tool_provider<Tools...>` 适合无状态 reflected tools。无状态 tool 的执行只依赖模型传入的 JSON 参数，框架可以从参数自动构造 tool 对象并调用 `invoke()`。
...
其中 `city` 是模型可填写的 tool argument。

Memory tools 属于有状态工具。它们除了模型传入的参数，还依赖应用预先绑定的运行时状态，例如：
...
这些状态不应由模型填写，也不应出现在 JSON schema 中。尤其是 `memory_scope`，如果让模型传入，就可能导致模型请求写入其他 user、tenant 或 application 的长期记忆。

因此 memory tools 使用 `memory_tool_provider` 保存 provider state：

```text
模型可填的字段 = tool arguments，例如 content、topic、limit
应用私有绑定 = provider state，例如 memory_context、scope、policy、review callback
```

`memory_tool_provider` 没有替代现有 Tool 机制，而是实现同样的 provider 协议：
...
因此它可以直接传给 `llm_agent_runner`。区别只是普通 reflected tools 由模型参数构造对象并直接调用无参 `invoke()`；memory tools 同样用 reflected aggregate 定义模型参数、生成 schema 并解析 JSON，但调用的是带 `memory_tool_context` 的 `invoke(context)`。provider 只负责注入应用私有状态。

`injected<T>` field marker 也是一种可选设计，能让有状态工具保持无参 `invoke()` 形式。但它需要 schema generation、argument parsing、required-field checking 和 provider injection mapping 全部理解 injected fields。当前 memory tools 选择 `invoke(context)`，是为了在 accuracy、generability、usability 和 simplicity 上保持更稳的第一版实现。详细取舍见 `docs/llm-tools.md` 的 Stateful Reflected Tools 章节。

推荐的运行链路：

```text
模型判断需要保存或查询记忆
-> 调用 save_memory 或 search_memory
-> 应用层工具绑定当前 memory_context 和 memory_scope
-> 工具校验 scope、policy、sensitivity 和参数
-> memory_context 写入或检索
-> tool result 返回给模型
```

Memory tools 需要解决的问题：

- 将长期记忆写入变成显式、可审计的动作，而不是模型或 runner 的隐式副作用。
- 让模型可以主动检索长期记忆，而不完全依赖本次 `augment()` 的自动召回结果。
- 保持长期记忆 opt-in。模型只能请求保存，应用策略仍然可以拒绝保存。
- 为后续 memory inspection、删除、审计和用户控制提供来源记录。
- 将长期记忆从开发者 API 能力推进为 agent runtime 能力。

Memory tools 不应让模型绕过框架安全边界：

- 不应允许空 scope 写入或搜索长期记忆。
- 不应允许模型直接指定任意 tenant/user/application scope。
- 不应保存 `sensitivity=secret` 的内容。
- 不应保存 access token、credential、原始 secrets 或应用未明确支持的敏感个人信息。
- 不应默认把所有对话内容自动转成长久记忆。
- 不应绕过应用配置的 review callback 或保留策略。

当前实现提供 `memory_tool_provider`，它绑定一个 `memory_context`，并暴露两个工具：

- `save_memory`：保存一条长期记忆。
- `search_memory`：搜索当前 scope 下的长期记忆。

`memory_tool_provider` 实现与普通 `tool_provider` 相同的 provider 接口：
...
auto memory_tools =
  std::make_shared<wuwe::agent::memory::memory_tool_provider>(memory);
...
expires_at_unix_millis: optional int64
```

`save_memory` 推荐行为：

- 使用应用绑定的当前 `memory_scope`，而不是信任模型传入 scope。
- 默认写入 `memory_kind::long_term`。
- 写入 metadata，例如 `source=tool`、`topic=<topic>`、`sensitivity=<sensitivity>`。
- 如果当前 scope 不满足 `policy.require_scope_for_long_term`，返回拒绝结果。
- 如果 `sensitivity=secret`，或应用策略判定内容敏感，返回拒绝结果。
- 如果应用配置 review callback，则在真正写入前调用 callback。

当前实现使用 `memory_context::remember()` 写入长期记忆，因此会复用已有长期记忆 scope 校验和 store 分流逻辑。模型不能通过工具参数指定 scope；工具只使用 provider 绑定的 scope，或 `memory_context::scope()`。
...
- 只搜索应用绑定的当前 scope。
- 默认只搜索 `memory_kind::long_term`。
- `limit` 必须受 `policy.max_long_term_records` 或工具自身上限约束。
- 不返回 `visibility=hidden` 或 `sensitivity=secret` 的记录。
- 返回内容应包含必要 metadata，但不应泄露内部 scope 或敏感字段。

当前实现返回 JSON array，每项包含 `id`、`kind`、`content` 和 `metadata`。搜索只查询 `long_term` records，`topic` 参数会映射为 metadata 精确过滤，`limit` 会被 `policy.max_long_term_records` 和 `memory_tool_options::max_search_results` 共同限制。
...
struct search_memory {
  static constexpr std::string_view description =
    "Search durable long-term memory records.";
...
`memory_tool_provider` 暴露 `save_memory` 和 `search_memory`。这两个工具的模型参数仍然是 reflected aggregate fields；`memory_context`、scope、policy 和 review callback 由 provider 作为私有状态注入，不出现在模型可填写的 JSON schema 中。
...
- `save_memory` 成功写入长期记忆。
- `search_memory` 按 query、topic 和 limit 搜索长期记忆。
- `save_memory` 拒绝 `sensitivity=secret`。
- `save_memory` 不允许绕过长期记忆 scope 策略。
- review callback 可以拒绝写入。
- `llm_agent_runner` 可以通过 `memory_tool_provider` 执行 tool call 并写入长期记忆。
...
- `memory_tool_provider` 已提供 `save_memory` 和 `search_memory`，模型可以通过显式 tool call 受控读写长期记忆。
- CTest 已覆盖 store、recall、augmentation、runner observation、文件持久化、SQLite 持久化和 memory tools 路径。

下一步应从“agent 能显式使用记忆”推进到“用户和应用能完整审计、演示和扩展记忆”。推荐按以下顺序推进：

1. **SQLite memory example**
...
2. **Memory inspection and deletion API**
...
- Memory tools 有测试覆盖，不会绕过长期记忆 scope 策略。当前 `memory_tool_provider` 已满足该项。

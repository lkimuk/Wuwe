...
第一版 SQLite 实现可以在 scoped filtering 后使用 C++ 侧词法排序。
...
2. 按 policy 检索候选记录。
3. 排除 hidden records。
4. 将记录格式化为 memory block。
5. 将 memory block 插入 request。
6. 返回修改后的 request。
...
- 第一次模型请求前，调用 `memory->augment(request, query_text)`。
...
第一版可以使用字符数预算。token-aware 计数可以后续加入。

预算算法：

1. 初始化 `remaining = policy.max_memory_chars`。
